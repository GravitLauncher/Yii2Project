...
<p>Этот протокол позворяет лаунчсерверу передавать события(events) подключенным клиентам вне зависимости от того, посылали они запрос или нет.</p>
<p>Скачивание ассетов и клиента проходит по протоколу http/https. Это позволяет подключить CDN и значительно ускорить загрузку файлов на машинах ваших пользователей</p>
...
<p>Благодаря возможности проксирования DDoS атаки на лаунчсервер значительно усложняются, а то и становятся невозможными</p>
<h2>Проксирование лаунчсервера через Nginx и CloudFlare</h2>
<p>Во избежании проблем создайте отдельный поддомен для лаунчсервера, например launcher.mysite.ru<br>
Не обязательно, но удобнее.</p>
...
<pre class="prettyprint">
    server {
        ...
        location / {
                root /путь/до/updates;
                autoindex on; #Добавьте эту настройку если хотите видеть листинг файлов.
        }
        location /api {
                proxy_pass http://localhost:9274/api;
                proxy_set_header Host $host;
